...
const ModelItems = ({ item }: { item: CMsItem }) => {
  ...
  return (
    <div className="text-[#333333] mb-3">
      ...
      {item.models
        .slice(0, isShowMore && item.models.length > 3 ? item.models.length : 3)
        .map((model) => {
          return (
            <ModelItem
              key={model.botName}
  ...
  return (
    <div
      ...
      <div className="flex items-center justify-start">
        ...
        {model.paidModel && (
          <Tooltip
            title={"This model needs to be paid at the provider’s website."}>
      ...
      {model.newModel && (
        <img src={NewTag} className="w-6 h-6 absolute right-0 top-0" alt="" />
      )}
    </div>
  )
}
