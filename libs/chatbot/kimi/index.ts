                  ...
                  if (event === "cmpl") {
                    ...
                    cb(
                      ...
                      new ConversationResponse({
                        ...
                        message_text:
                          _this.convertToSuperscript(outputText) + textAppendix,
                    ...
                    _this.botSession.session.addMessage(
                      new SimpleBotMessage(text, messageId)
                    )
                  } else if (event === "ref_docs") {
                    if (ref_docs && ref_docs.length) {
                      const { links: _links, tags: _tags } =
                        _this.getRefDocsMarkdown(ref_docs, refLinks, refTags)
                      refLinks = _links
                      refTags = _tags

                      const tags = Array.from(new Set(refTags)).sort(
